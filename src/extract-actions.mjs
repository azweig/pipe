...
{"todos":[{"tarea":"algo concreto que me pidieron o que quedó de MI lado por hacer","cuando":"plazo/fecha si se menciona, si no vacío"}],
 "promesas":[{"promesa":"algo que YO me comprometí a hacer y todavía falta","cuando":"plazo si hay, si no vacío"}]}
Reglas estrictas: solo cosas ACCIONABLES y aún pendientes; máximo 2 de cada; si no hay, array vacío; NO inventes ni supongas nada que no esté escrito.`
  ...
  for (const t of (r?.todos || []).slice(0, 2)) {
    ...
    if (insertTodo(keyOf("todo", th.thread, txt), txt, th.thread, who, (t.cuando || "").slice(0, 40), th.mx, NOW).changes) nT++
  }
  for (const p of (r?.promesas || []).slice(0, 2)) {
    const txt = (p.promesa || "").trim(); if (txt.length < 4) continue
    if (insertPromesa(keyOf("prom", th.thread, txt), txt, th.thread, who, (p.cuando || "").slice(0, 40), th.mx, NOW).changes) nP++
  }
}

setMeta("extract_last_ts", String(maxTs))
console.log(`[extract] ${threads.length} hilos · ${nT} tareas · ${nP} promesas nuevas`)
process.exit(0)
