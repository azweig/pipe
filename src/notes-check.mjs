...
import { isSecretSelfNote } from "./lib/secret.mjs" // 🔒 no imprimir texto de notas de canal secreto
const h = handle()
...
const s = h.prepare("SELECT m.text, n.title, m.channel, m.account, m.jid FROM note_meta n JOIN messages m ON m.id=n.id WHERE n.title LIKE '%Facebook%' AND n.status='active' LIMIT 5").all().filter((x) => !isSecretSelfNote(x))
