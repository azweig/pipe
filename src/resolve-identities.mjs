...
import { rekeyContacts, rekeyBridge, rekeyManual, rekeyEmails, rekeyPushNames, unifyByNumber, pruneOrphanConversations } from "./lib/db.mjs"
...
const w = rekeyPushNames(contacts, manual)  // sin agenda: usa el nombre que la persona se puso en WhatsApp (con guardas anti-homónimo)
const p = pruneOrphanConversations()        // borra conversaciones fantasma (thread ya sin mensajes tras el re-key)
console.log(`[resolve] contactos:${c} bridge:${b} emails:${e} unify:${JSON.stringify(u)} manual:${m} pushnames:${w} poda:${JSON.stringify(p)}`)
