...
// ¿estamos en un teléfono? El botón de cámara solo tiene sentido ahí (en escritorio el picker ya alcanza).
const _esMovil = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent) || (navigator.maxTouchPoints > 1 && /Mac/.test(navigator.platform))
...
window.pickContacto = async () => {
  const d = await api("/api/threads?limit=600") || []
  const items = (Array.isArray(d) ? d : d.items || []).filter((t) => !t.group && !t.espacio && t.key !== "self" && t.name)
  openSheet(`<h2 style="margin:0 0 2px">Enviar un contacto</h2>
    <div class="sub" style="margin:0 0 10px">Se manda como tarjeta: lo pueden guardar en la agenda de una.</div>
    <input id="ctBuscar" placeholder="Buscar…" oninput="ctFiltrar(this.value)" style="${CINP};margin-bottom:8px">
    <div id="ctLista" style="max-height:46vh;overflow:auto"></div>`)
  window._ctItems = items
  ctFiltrar("")
}
window.ctFiltrar = (q) => {
  const n = String(q || "").toLowerCase()
  const l = (window._ctItems || []).filter((t) => !n || (t.name || "").toLowerCase().includes(n)).slice(0, 40)
  const el = document.getElementById("ctLista"); if (!el) return
  el.innerHTML = l.length ? l.map((t) => `<div class="card itemtap" style="padding:9px 11px;display:flex;align-items:center;gap:9px;cursor:pointer" onclick="enviarContacto(${escj(t.name)})">${avatar(t.name, t.photo, "sm")}<div class="b">${esc(t.name)}</div></div>`).join("") : `<div class="sub">Sin resultados</div>`
}
window.enviarContacto = async (nombre) => {
  closeSheet(); flash("enviando contacto…")
  const r = await post("/api/send-contact", { key: convState.key, contacto: nombre, channel: convState.target?.channel, target: convState.target?.target })
  flash(r && r.error ? "❌ " + r.error : `👤 ${nombre} enviado`)
  if (r && !r.error) refreshConv(convState.key)
}
